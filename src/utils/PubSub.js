...
class PubSub {
  ...
  subscribe = (topic, func) => {
    ...
    console.log(`subscribe topic:${topic} subId:${token} desc:${TopicsEnum.properties[topic]?.desc}`);
    return { topic, token };
  };
  ...
  publish = (topic, ...args) => this.topics[topic]?.forEach((subscription) => subscription.func(...args));
}
